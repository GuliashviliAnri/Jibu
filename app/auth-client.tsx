...
export function useBrokerAccess() {
  ...
  useEffect(() => {
    ...
    const sync = async () => {
      ...
      try {
        const status = await brokerAccessStatus(user.id);
        if (disposed) return;
        setOwner(status.active ? user.id : null);
        const expires = status.expiresAt
          ? new Date(status.expiresAt).getTime()
          : 0;
        if (status.active && expires > Date.now())
          timer = setTimeout(
            () => void sync(),
            Math.min(expires - Date.now() + 50, 2147483647),
          );
      } catch {
        if (!disposed) setOwner(null);
      }
    };
    const refresh = () => void sync();
    void sync();
    window.addEventListener("jibu:subscription", refresh);
    window.addEventListener("focus", refresh);
    document.addEventListener("visibilitychange", refresh);
    return () => {
      disposed = true;
      if (timer) clearTimeout(timer);
      window.removeEventListener("jibu:subscription", refresh);
